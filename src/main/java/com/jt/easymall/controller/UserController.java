...
public class UserController {
	...
	/*
	 * 请求url：http://localhost/user_ajax/checkUserName 请求方式: post 请求参数: String
	 * userName; 备注: select * from t_user where user_name=#{userName};
	 * 如果返回值非空,说明当前用户名已经被使用了 如果为空才可以使用; 返回值:SysResult对象,转化json(@ResponseBody)
	 * status:0表示可用,1表示已存在
	 */
	@Autowired
	...
	/*
	 * 接口文件 请求url:http://localhost/user_ajax/regist 请求方式:post 请求参数:User user
	 * 返回数据:SysResult的对象json字符串,1表示注册成功,0表示失败 返回成功之后,页面跳转到/page/login
	 */
